...
var getViewForSwagger2 = function(opts, type){
    ...
    _.forEach(swagger.definitions, function(typeInfo, typeName) {
        ...
        data.methods.forEach(function(method){
            ...
            let methodNameParts = method.methodName.split('_');
            let methodName;

            if(methodNameParts[0] === tsType.name){
                //tsType.methods.push({method: method});
                if(methodNameParts[1] && methodNameParts[1] !== 'prototype'){
                    methodName = methodNameParts[1];
                }else if (methodNameParts[2] && methodNameParts[2]){
                ...
                if(methodName){
                    //console.log('adding ' + methodName + ' to ' + tsType.name + ' as', method);
...
var getCode = function(opts, type) {
    ...
    if (type === 'custom') {
        ...
        if(type === 'typescript' || type === 'typescriptTypes') {
            console.log('some kinda typescript');
            opts.template.type = fs.readFileSync(templates + 'type.mustache', 'utf-8');
            if(type === 'typescriptTypes'){
                opts.template.class = fs.readFileSync(templates + 'typescript-definitions-only.mustache', 'utf-8');
                opts.template.method = fs.readFileSync(templates + 'typescript-definitions-only.mustache', 'utf-8');
            }
        }else{
